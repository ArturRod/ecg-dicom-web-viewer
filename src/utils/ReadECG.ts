...
const {DicomMetaDictionary, DicomMessage, ReadBufferStream, WriteBufferStream} =
	dcmjs.data;
...
class DicomEcg {
	...
	getElement(tag) {
		...
		if (tag === "WaveformSequence" && !this.elements.WaveformSequence) {
			const alt =
				this.elements["54000100"] ||
				this.elements["5400,0100"] ||
				this.elements["(5400,0100)"];
			if (alt) {
	...
	getDenaturalizedDataset() {
		const denaturalizedDataset = DicomMetaDictionary.denaturalizeDataset(
			this.getElements()
		);
	...
	_fromP10Buffer(arrayBuffer) {
		...
		try {
			...
			if (waveSeqOffset > 0) {
				...
				for (const ts of syntaxCandidates) {
					...
					if (elems && (elems.WaveformSequence || elems["54000100"])) {
						console.warn(
							"Recovered dataset via heuristic WaveformSequence scan at offset",
							waveSeqOffset,
							"syntax",
							ts
						);
			...
			for (let offset = 4; offset < Math.min(u8.length - 4, 1024); offset++) {
				if (
					...
					const sliced = arrayBuffer.slice(
						offset - 128 > 0 ? offset - 128 : offset
					);
					for (const ts of syntaxCandidates) {
						const elems = tryRaw(sliced, ts);
						if (elems) {
							console.warn(
								"Recovered dataset after locating delayed DICM magic at offset",
								offset
							);
							return {elements: elems, transferSyntaxUid: ts};
						}
			...
			try {
				...
				const meta2 = DicomMetaDictionary.naturalizeDataset(
					dicomDict2.meta || {}
				);
				const ts2 = meta2.TransferSyntaxUID || "1.2.840.10008.1.2";
				const elements2 = DicomMetaDictionary.naturalizeDataset(
					dicomDict2.dict || {}
				);
	...
	_fromElementsBuffer(arrayBuffer, transferSyntaxUid) {
		...
		let syntaxLengthTypeToDecode =
			transferSyntaxUid === "1.2.840.10008.1.2"
				? "1.2.840.10008.1.2"
				: "1.2.840.10008.1.2.1";
		const denaturalizedDataset = DicomMessage._read(
			stream,
			syntaxLengthTypeToDecode,
			{
				ignoreErrors: true,
			}
		);
	...
	_postProcessElements() {
		...
		if (direct && !this.elements.WaveformSequence) {
			...
			console.warn(
				"WaveformSequence recovered via direct numeric tag 54000100 (top-level)."
			);
			return;
		}
		...
		if (deep && !this.elements.WaveformSequence) {
			...
			console.warn(
				"WaveformSequence recovered via deep recursive search for tag 54000100."
			);
		}
	}
	...
	_extractWaveform() {
		...
		const candidates = waveformSequence.filter(
			item =>
				item &&
				item.WaveformSampleInterpretation === "SS" &&
				item.WaveformBitsAllocated === 16
		);
		let waveformSequenceItem = candidates.sort(
			(a, b) => (b.NumberOfWaveformSamples || 0) - (a.NumberOfWaveformSamples || 0)
		)[0];
	...
	_calculateLeads(waveform) {
		...
		channelDefinitionSequence.forEach((channelDefinitionSequenceItem, i) => {
			if (channelDefinitionSequenceItem !== undefined) {
				//Bits:
				if (
					!WAVE_FORM_BITS_STORED.includes(
				...
				if (
					...
					const channelSensitivityUnitsSequenceFirstItem =
						channelSensitivityUnitsSequence[0];
					if (
						channelSensitivityUnitsSequenceFirstItem.CodeValue !== undefined
					) {
						units.push(channelSensitivityUnitsSequenceFirstItem.CodeValue);
					}
				}

				const channelSourceSequence =
				...
				if (
					...
					channelSourceSequence.forEach(channelSourceSequenceItem => {
						...
						const schemeDesignator =
							channelSourceSequenceItem.CodingSchemeDesignator;
						if (
							!title &&
							codeValue !== undefined &&
							...
							if (schemeDesignator === "MDC") {
								const mdcCodeTitle = mdcScpEcgCodeTitles.find(
									i => i.mdcCode === codeValue
								);
								if (mdcCodeTitle !== undefined) {
									title = mdcCodeTitle.title;
								}
							} else if (schemeDesignator === "SCPECG") {
								const scpEcgCodeTitle = mdcScpEcgCodeTitles.find(
									i => i.scpEcgCode === codeValue
								);
	...
	_extractInformation(waveform) {
		...
		waveformAnnotationSequence.forEach(waveformAnnotationSequenceItem => {
			...
			if (
				...
				conceptNameCodeSequence.forEach(conceptNameCodeSequenceItem => {
					...
					let cleanedCodeMeaning =
						conceptNameCodeSequenceItem.CodeMeaning.replace(/\u0000/g, "");
					cleanedCodeMeaning = this._normalizeCodeMeaning(cleanedCodeMeaning);
					//Search constant data:
					const keyUnitInfo = KEY_UNIT_INFO.find(
						i => i.key === cleanedCodeMeaning
					);
					//Load data:
					if (
		...
		if (!info.find(i => i.key === "VRATE") && rrInterval) {
			info.push({
				key: "VRATE",
				value: Math.trunc(
					((60.0 / waveform.duration) * waveform.samples) / rrInterval.value
				),
				unit: "BPM",
			});
		}
	...
	_debugDumpFirstTags(u8: Uint8Array, count: number = 12) {
		try {
			...
			for (let i = 0; i < u8.length - 8 && tags.length < count; ) {
				...
				const tagStr = `(${g.toString(16).padStart(4, "0")},${e
					.toString(16)
					.padStart(4, "0")})`;
