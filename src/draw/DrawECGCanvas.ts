...
class DrawECGCanvas extends GenericCanvas {
  ...
  private buttonsEvents() {
    ...
    buttonReset.addEventListener('click', () => this.resetView());

    //Center:
    let buttonCenter = document.getElementById('centerButton');
    buttonCenter.addEventListener('click', () => this.centerView());
    ...
    //Context Menu Items:
    let contextCenter = document.getElementById('contextCenter');
    contextCenter.addEventListener('click', () => {
      this.centerView();
      this.hideContextMenu();
    });

    let contextReset = document.getElementById('contextReset');
  ...
  private changeZoom(min: boolean) {
    ...
    if (min) {
      ...
      if (this.configuration.CURVE_WIDTH < 1.5 && this.scale < 3.8) {
  ...
  /**
   * Reset view to default values (zoom, pan, amplitude, time, etc.)
   */
  private resetView() {
    // Reset scale (zoom) to default value
    this.scale = 1;
    ...
    // Reset amplitude to default (10mm/mV)
    this.amplitude = 0.1;

    // Reset time/speed to default (25mm/s)
    this.time = 0.25;

    // Reset curve width to default
    this.configuration.CURVE_WIDTH = 1.5;

    // Update UI text displays
    let textAmplitude = document.getElementById('textAmplitude');
    textAmplitude.innerText = ' ' + Math.round(0.1 * 100) + 'mm/mV ';

    let textTime = document.getElementById('textTime');
    textTime.innerText = ' ' + Math.round(0.25 * 100) + 'mm/s ';
  ...
  /**
   * Center the view by resetting only pan/offset values
   */
  private centerView() {
  ...
  private GetCurvePoints(points) {
    ...
    for (let i = 2; i < points.length - 4; i += 2) {
      ...
      for (let t = 0; t <= SPLINE.numOfSegments; t++) {
        ...
        result.push(c1 * points[i] + c2 * points[i + 2] + c3 * t1x + c4 * t2x, c1 * points[i + 1] + c2 * points[i + 3] + c3 * t1y + c4 * t2y);
      }
    }
    return result;
  }
  ...
  private ReadObjPosition(position) {
    ...
    if (code == undefined) {
      ...
      if (codeMeaning != undefined) {
        if (codeMeaning.split(' ').length === 1) {
          //Comes in correct format
          code = codeMeaning;
        } else if (codeMeaning.split(' ').length === 2 || codeMeaning.split(' ').length === 3) {
          //3 text + Bethoven:
      ...
      if (code.split(' ').length === 2 || code.split(' ').length === 3) {
        //3 text + Bethoven:
        code = code.split(' ')[1];
      } else if (code.split('_').length === 2 || code.split('_').length === 3) {
        //3 text + Bethoven or _ separator:
        code = code.split('_')[1];
      }
    }
